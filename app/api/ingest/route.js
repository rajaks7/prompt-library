...
export async function POST(req) {
  try {
    ...
    let ai_tool_id = null;
    if (row.ai_tool_name) {
      const { data: tools, error: tErr } = await supa.from('ai_tools').select('id,name').ilike('name', row.ai_tool_name).limit(1);
      if (tErr) throw tErr;
    ...
    if (row.category_name) {
      const { data: cats, error: cErr } = await supa.from('categories').select('id,name').ilike('name', row.category_name).limit(1);
      if (cErr) throw cErr;
      if (!cats || !cats.length) {
    ...
    let type_id = null;
    if (row.type_name) {
      const { data: types, error: tpErr } = await supa.from('prompt_types').select('id,name').ilike('name', row.type_name).limit(1);
