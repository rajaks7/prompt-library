...
export default function LibraryClient({ 
  ...
  // Step 1: shared style for Sort / Filter selects
  ...
  cursor: 'pointer',
  color: '#374151',
  minWidth: windowWidth < 640 ? '110px' : '180px',
  ...
  console.log('Filtered result count:', filteredPrompts.length)
}, [filters, filteredPrompts])
...
useEffect(() => {
  if (filters.search) {
    ...
    const searchResults = prompts.filter(p =>
      ...
      (p.ai_tool_model || '').toLowerCase().includes(filters.search.toLowerCase()) ||
      (Array.isArray(p.tags) && p.tags.some(tag => tag.toLowerCase().includes(filters.search.toLowerCase())))
    )
    console.log('Search results:', searchResults.map(p => ({ id: p.id, title: p.title })))
  }
  
  if (filters.category) {
    console.log('CATEGORY DEBUG:')
    console.log('Selected category:', filters.category)
    const categoryResults = prompts.filter(p => p.category_name === filters.category)
    console.log('Category results:', categoryResults.map(p => ({ id: p.id, title: p.title, category: p.category_name })))
  }
}, [filters, prompts])
  ...
  const uniqueTools = tools.map(t => t.name)
        ...
        <div style={{ 
          ...
          <div style={{ 
            ...
            <div style={{
              ...
              {(() => {
                const hasActiveFilters = filters.search || filters.tool || filters.category || filters.rating || filters.favoritesOnly
                return hasActiveFilters ? filteredPrompts.length : totalCount
              })()} prompts
            </div>
          </div>
      ...
      <div style={{
            ...
            <div style={{ position: 'relative', width: '100%' }}>
              <div style={{ 
                ...
                <input
                  type="text"
                  name="search"
                  placeholder={windowWidth < 640 ? "Search prompts..." : "Search prompts, tags, or content..."}
        ...
        <div style={{
          ...
          <div style={{ 
            ...
            {/* Sort dropdown (keeps existing name/value/handler) */}
            <select
              name="sort"
              value={filters.sort}
              onChange={handleFilterChange}
              style={{
                ...
                cursor: 'pointer',
                color: '#4a5568',
                minWidth: windowWidth < 640 ? '120px' : '220px',
                boxShadow: '0 6px 18px rgba(16,24,40,0.06)',
                outline: 'none'
              }}
            >
            ...
            <select
              ...
              style={{
                ...selectControlStyle,
                minWidth: windowWidth < 640 ? '110px' : '160px'
              }}
            >
            ...
            {/* Category filter (use DB categories) */}
            <select
              name="category"
            ...
            <select
              ...
              style={{
                ...selectControlStyle,
                minWidth: windowWidth < 640 ? '110px' : '140px'
              }}
            >
            ...
            {/* Clear button (keeps your existing conditional) */}
                        ...
                        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                          <div style={{ 
                            ...
                            {p.attachment_filename ? (
                              <img
                                src={
                                  (p.attachment_filename || '').startsWith('http')
                                    ? p.attachment_filename
                                    : `https://amfwuqwcivfptdfgpcsw.supabase.co/storage/v1/object/public/prompt-assets/${p.attachment_filename}`
                                }
                                alt={p.title}
                                onError={(e) => {
                                  // fallback to category image if exists, otherwise a tiny transparent image
                                  const fallback = (p.category_image_url && p.category_image_url.startsWith('http'))
                          ...
                          <div style={{ minWidth: 0 }}>
                            ...
                            <div style={{ 
                              ...
                              {(() => {
                                ...
                                const limit = windowWidth < 640 ? 40 : 80
